...
console.log('🟢🟢🟢 Rewrite Script loaded 🟢🟢🟢');
...
function updateAllXY() {
  ...
  FrameX -= offsetX;
  FrameY -= offsetY;
  // Then clamp to be safe

  FrameX = Math.max(0, Math.min((frame.getBoundingClientRect().width - 1)/zoom, FrameX));
  FrameY = Math.max(0, Math.min((frame.getBoundingClientRect().height - 1)/zoom, FrameY));
}
